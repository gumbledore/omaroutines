...
def test_model_forwarded_to_herdr_claude(herdr_cli, state_home, cwd_dir, stub_dir):
    add_task(herdr_cli, "t1", cwd_dir, worktree="false", agent="claude", model="opus")
    assert herdr_cli("trigger", "t1").returncode == 0
    args = (stub_dir / "start-args.t1-1").read_text().splitlines()
    assert args == ["--permission-mode", "auto", "--model", "opus"]
